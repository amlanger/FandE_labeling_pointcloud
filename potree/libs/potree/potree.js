...
//Um doppelte Updated zu vermeiden #Spaghetticode
var already_updated_seen = "";
...
Potree.InputHandler = class InputHandler extends THREE.EventDispatcher {
    ...
    storeVolume(){
        let all_visible_pointclouds = ""
        for (i = 0; i < this.scene.pointclouds.length; i++) {
            if (this.scene.pointclouds[i].visible) {
                all_visible_pointclouds = all_visible_pointclouds + this.scene.pointclouds[i].name
                //alert(all_visible_pointclouds);
            }
        }


        for (i = 0; i < this.selection.length; i++) {
            if (this.selection[i] instanceof Potree.Volume) {
                let volume = this.selection[i];
                if (!volume._removed){
                    viewer.volumeTool.inserNewLabel({
                        id: volume._id,
                        class: volume.name.toString(),
                        utmx: volume.label.position.x,
                        utmy: volume.label.position.y,
                        utmz: volume.label.position.z,
                        geomx: volume.scale.x,
                        geomy: volume.scale.y,
                        geomz: volume.scale.z,
                        rotx: volume.rotation.x,
                        roty: volume.rotation.y,
                        rotz: volume.rotation.z,
                        cloudid: all_visible_pointclouds
                        //alert(all_visible_pointclouds);
                    })
                }
    ...
    function onKeyDown(event) {
        ...
        switch (event.keyCode) {
            ...
            case scope.keys.E:
                ...
                break;
            case scope.keys.R:
                scope.raiseCamera = true;
                break;
            case scope.keys.F:
                scope.lowerCamera = true;
                break;
        }
    }
    ...
    function onKeyUp(event) {
        ...
        switch (event.keyCode) {
            ...
            case scope.keys.Q:
                ...
                break;
            case scope.keys.E:
                scope.rotRight = false;
                break;
            case scope.keys.R:
                scope.raiseCamera = false;
                break;
            case scope.keys.F:
                scope.lowerCamera = false;
                break;
        }
    }
...
Potree.PointCloudGreyhoundGeometryNode = function (name, pcoGeometry, boundingBox, scale, offset) {
...
Potree.MeasuringTool = class MeasuringTool extends THREE.EventDispatcher {
    ...
    constructor(viewer) {
        ...
        this.onAdd = e => {
            this.sceneMeasurement.add(e.measurement)
        };
    }
    ...
    constructor(viewer) {
        ...
        this.viewer.inputHandler.addEventListener("keydown", (e) => {
            if (this.selection.length > 0) {
                ...
                }else if (this.keys.CLIP.some(key => key === e.keyCode)) {
                    for (i = 0; i < this.selection.length; i++) {
                        if (this.selection[i] instanceof Potree.Volume) {
                            if (this.selection[i]._clip === true) {
                                this.selection[i]._clip = false;
                            } else {
                                this.selection[i]._clip = true;
                            }
        ...
        { // Menu
            ...
            this.menu.addItem(new HoverMenuItem(Potree.resourcePath + "/icons/clip_volume.svg", e => {
                //console.log("scale!");
                for (i = 0; i < this.selection.length; i++) {
                    if (this.selection[i] instanceof Potree.Volume) {
                        if (this.selection[i]._clip === true) {
            ...
            this.menu.addItem(new HoverMenuItem(Potree.resourcePath + "/icons/remove.svg", e => {
                //console.log("rotate!")
                for (i = 0; i < this.selection.length; i++) {
                    if (this.selection[i] instanceof Potree.Volume) {
                        viewer.scene.removeVolume(this.selection[i]);
                    }
                }
                this.selection = [];
...
Potree.Volume = class extends THREE.Object3D {
    ...
    constructor(args = {}) {
        ...
        this._removed = false;
        let boxGeometry = 0;
        let boxFrameGeometry = 0;
        var boxColor = 0x00ff00;
        if (this.name === "vegetation") {
            boxGeometry = new THREE.BoxGeometry(1,1,1);
            boxGeometry.computeBoundingBox();
            boxFrameGeometry = new THREE.BoxGeometry(1,1,1);
            boxColor = 0x6b8e23;
            this.scale.x=8;
            this.scale.y=8;
            this.scale.z=8;
            ...
            this.scale.y=0.33;
            this.scale.z=1;
        } else if (this.name === "trafficlight") {
            boxGeometry = new THREE.BoxGeometry(1,1,1);
            boxGeometry.computeBoundingBox();
            boxFrameGeometry = new THREE.BoxGeometry(1,1,1);
            boxColor = 0xfaaa1e;
...
Potree.VolumeTool = class VolumeTool extends THREE.EventDispatcher {
    ...
    //    volume.position.x = args.posx;
    //    volume.position.y = args.posy;
    //    volume.position.z = args.posz;
    //    volume.scale.x = args.scalex;
    //    volume.scale.y = args.scaley;
    //    volume.scale.z = args.scalez;
    //    volume.rotation.x = args.rotx;
    //    volume.rotation.y = args.roty;
    //    volume.rotation.z = args.rotz;
    //
    //    this.dispatchEvent({
    ...
    //
    //
    //    // let volume = new Potree.Volume({name:args.name});
    //    // volume.clip = args.clip || false;
    //    // volume.name = args.name || "Volume";
    //    //
    //    // //this.sceneVolume.add(volume);
    ...
    inserNewLabel(args = {}) {
        if (true) {
            $.post("php2/insert.php", {
                id: args.id.toString(),
                class: args.class.toString(),
                utmx: args.utmx.toString(),
                utmy: args.utmy.toString(),
                utmz: args.utmz.toString(),
                geomx: args.geomx.toString(),
                geomy: args.geomy.toString(),
                geomz: args.geomz.toString(),
                rotx: args.rotx.toString(),
                roty: args.roty.toString(),
                rotz: args.rotz.toString(),
                cloudid: args.cloudid.toString()
    ...
    removeElemFromDB(args = {}) {
        //removeMeasurement
        //viewer.volumeTool.removeElemFromDB()
        $.post("php2/deleterow.php", {
            id: args.id.toString(),
        })
        //.done(function( data ) {
        //alert( "Data Loaded: " + data );
    ...
    //     measurements = measurements.filter(m => m instanceof Potree.Volume);
    //
    //     for (i = 0; i < measurements.length; i++) {
    //         let volume = measurements[i];
    //         viewer.volumeTool.inserNewLabel({
    //             id: volume._id,
    //             class: volume.name.toString(),
    //             utmx: volume.label.position.x,
    ...
    //var all_visible_pointclouds2 = ""
    //if(this.scene.pointclouds.length){
    //  for (j = 0; j < this.scene.pointclouds.length; j++) {
    //      if (this.scene.pointclouds[j].visible) {
    //          all_visible_pointclouds = all_visible_pointclouds + this.scene.pointclouds[j].name
    //      }
    //  }
    //}
    ...
    addElemsFromDB() {
      ...
      //check ob Punktwolken drin sind und falls ja welche angeklickt ist
      // NUR eine möglich!! -> diese wir in seen abgespeichert
        if(this.scene.pointclouds.length){
          let mz;
          var seen;
          for(mz = 0; mz < this.scene.pointclouds.length; mz++){
            if(this.scene.pointclouds[mz].visible){
                seen = this.scene.pointclouds[mz].name;
              }
          }
        }
        ...
        if(seen){
          //already_updated_seen = nur damit bei mehrmaligem update-klicken nicht die gleichen Daten reingeladen werden
            if(already_updated_seen !== seen){

            $.ajax({
                type: "GET",
                url: "php2/getalldata.php",
                success: function (data) {

                    let arr = data.split(" ")
                    //Hier wird jedes Objekt einzeln aus der Datenbank extrahiert
                    for (i = 0; i < arr.length / 12 - 1; i++) {
                        let id = arr[i * 12];
                        let cl = arr[i * 12 + 1];
                        let utmx = arr[i * 12 + 2];
                        let utmy = arr[i * 12 + 3];
                        let utmz = arr[i * 12 + 4];
                        let geomx = arr[i * 12 + 5];
                        let geomy = arr[i * 12 + 6];
                        let geomz = arr[i * 12 + 7];
                        let rotx = arr[i * 12 + 8];
                        let roty = arr[i * 12 + 9];
                        ...
                        //ab hier wird geprüft ob das Objekt zur passenden Punktwolke gehört
                        // oder nicht
                        let add = false;
                        if(seen === cloudid){
                        ...
                        //Falls add = true wird das Objekt hinzugefügt
                        if (add) {
                            ...
                            for (let measure of measurements) {
                                if (id == measure._id) {
                                    measure.position.x = parseFloat(utmx);
                                    measure.position.y = parseFloat(utmy);
                                    measure.position.z = parseFloat(utmz);
                                    measure.scale.x = parseFloat(geomx);
                                    measure.scale.y = parseFloat(geomy);
          ...
          } //Ende if(already_updated_seen)
          already_updated_seen = seen;
        } //ENDE if(seen)
...
/*
        $.ajax({
            ...
            success: function (data) {
                ...
                for (i = 0; i < arr.length / 12 - 1; i++) {
                    ...
                    for (let measure of measurements) {
                        //window.alert(measure.toSource())

                        if (id == measure._id) {
                        //if (cloudid == seen) {
                          window.alert(cloudid)
                        //alert(this.scene.pointclouds.name);
                        //if (cloudid == this.scene.pointclouds.name) {
                            add = false;
                        }
                    }
                    if (add) {
    ...
    //
    //         //alert_data(obj.labels[0].position.x);
    //         //viewer.volumeTool.makeOldElemet({name: "Tree"})
    //         for (var i = 0; i < obj.labels.length; i++){
    //             viewer.volumeTool.makeOldElemet({
    // 			name: obj.labels[i].class,
    // 			posx:obj.labels[i].position.x,
    // 			posy:obj.labels[i].position.y,
    // 			posz:obj.labels[i].position.z,
    //                 scalex:obj.labels[i].geometry.length,
    //                 scaley:obj.labels[i].geometry.width,
    //                 scalez:obj.labels[i].geometry.height,
    //                 rotx:obj.labels[i].rotation.x,
    //                 roty:obj.labels[i].rotation.y,
    //                 rotz:obj.labels[i].rotation.z
    //             })
    //         };
    // 	//alert_data(obj.labels[0].class);
    //     }
    //
    ...
    startInsertion(args = {}) {
        ...
        if (args.setValues) {
            ...
            volume.position.z = parseFloat(args.posz);
            volume.scale.x = parseFloat(args.scalex);
            volume.scale.y = parseFloat(args.scaley);
            volume.scale.z = parseFloat(args.scalez);
            volume.rotation.x = parseFloat(args.rotx);
            volume.rotation.y = parseFloat(args.roty);
            volume.rotation.z = parseFloat(args.rotz);
        }
        ...
        let drag = e => {
            ...
            if (I) {
                //window.alert(I.location.x+" "+I.location.y+" "+I.location.z);
                volume.position.copy(I.location);



                // viewer.volumeTool.inserNewLabel({id: volume._id,
                //     class: args.name.toString(),
                //     utmx: volume.label.position.x,
                //     utmy: volume.label.position.y,
                //     utmz: volume.label.position.z,
                //     geomx: volume.scale.x,
        ...
        let drop = e => {
            ...
            volume.removeEventListener("drag", drag);
            volume.removeEventListener("drop", drop);
            this.viewer.volumeTool.inserNewLabel({
                id: volume._id,
                class: volume.name.toString(),
                utmx: volume.label.position.x,
                utmy: volume.label.position.y,
    ...
    update(delta) {
        ...
        for (let volume of volumes) {
            ...
            let text = Potree.utils.addCommas(volume.name);
            label.setText(text);
...
Potree.GeoJSONExporter = class GeoJSONExporter {
    static measurementToFeatures(measurement) {
        ...
        let feature = {
            ...
            position: measurement.label.position,
            geometry: {
                length: measurement.scale.x,
                width: measurement.scale.y,
                height: measurement.scale.z
            },
            rotation: {
                x: measurement.rotation.x,
                y: measurement.rotation.y,
                z: measurement.rotation.z
            }
        };
...
Potree.Scene = class extends THREE.EventDispatcher {
    ...
    removeVolume(volume) {
        volume._removed=true;
        viewer.volumeTool.removeElemFromDB({id: volume._id});
...
Potree.Viewer = class PotreeViewer extends THREE.EventDispatcher {
    ...
    constructor(domElement, args) {
        ...
        this.fov = 60;
        this.clipMode = Potree.ClipMode.CLIP_OUTSIDE;
        this.isFlipYZ = false;
        ...
        {// set defaults
            ...
            this.setPointBudget(5 * 1000 * 1000);
...
let createUploadIcon = function (icon, title, callback) {
    let element = $(`
...
let createToolButton = function (icon, title, callback) {
    let element = $(`
...
function initToolbar() {
    ...
    // elToolbar.append(createToolButton(
    //     "upload",
    //     "[title]tt.volume_measurement",
    //     function(){viewer.volumeTool.addAllVolumesDB()},
    //     //function(){viewer.volumeTool.makeOldElemet({name: "Tree"})}
    // ));
    //
    //
    elToolbar.append(createToolButton(
    ...
    // elToolbar.append(createUploadIcon(
    //     Potree.resourcePath + "/icons/tree.jpg",
    //     "[title]tt.volume_measurement",
    //     function(){viewer.volumeTool.uploadFile()},
    //     //function(){viewer.volumeTool.makeOldElemet({name: "Tree"})}
    // ));


    let classes = ["road", "sidewalk", "building", "wall", "fence", "pole", "trafficlight", "trafficsign", "vegetation", "terrain", "person", "car", "truck", "bus", "train", "motorcycle", "bicycle"]
    //let classes  = ["Tree","Car","Street"]
    for (var i = 0; i < classes.length; i++) {
        ...
        elToolbar.append(createToolButton(
            ...
            "[title]tt.volume_measurement",
            function () {
                viewer.volumeTool.getNewID(temp)
            }
        ));
    }
    ...
    // 	Potree.resourcePath + "/icons/Car.png",
    // 	"[title]tt.volume_measurement",
    // 	function(){viewer.volumeTool.startInsertion({name: "Car"})}
    // ));
    //
    // elToolbar.append(createToolIcon(
    // 	Potree.resourcePath + "/icons/map.png",
    // 	"[title]tt.volume_measurement",
    // 	function(){viewer.volumeTool.startInsertion({name: "Street"})}
    // ));
            ...
            { // ACTIONS, INCLUDING GOTO LOCATION
                if (annotation.hasView()) {
                    let action = new Potree.Action({
                        ...
                        "onclick": (e) => {
                            annotation.moveHere(viewer.scene.camera)
                        }
                    });
...
function initMeasurementDetails() {
    ...
    class DistancePanel extends MeasurePanel {
        constructor(scene, measurement) {
            ...
            elRemove.click(() => {
                this.scene.removeMeasurement(measurement)
            });
    ...
    class ProfilePanel extends MeasurePanel {
        constructor(scene, measurement) {
            ...
            let elRemove = this.elContent.find(".measurement_action_remove");
            elRemove.click(() => {
                this.scene.removeProfile(measurement)
            });
